...
//					})
			...
			var i = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a>',//<a id="save" class="ui-btn">保存</a>
				...
				a = '<a id="edit" class="ui-btn">保存</a>',/*<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="#" target="_blank" id="print" rel="'+ t.id +'" class="ui-btn">打印</a>*/
...
//			}, 
			...
			$("#grid").jqGrid({
				...
				afterSaveCell: function(t, i, a, r, n) {
					...
					case "qty":
                        var a = parseFloat(a);
						if(notshow === 0) {//不展示金额列选项供填写
                            var h = parseFloat($("#grid").jqGrid("getCell", t, n + 4)),//销售单价
					...
					case "discountRate":
						var a = parseFloat(a),
							m = parseFloat($("#grid").jqGrid("getCell", t, n - 4)),
					...
					case "deduction":
						var a = parseFloat(a),
							m = parseFloat($("#grid").jqGrid("getCell", t, n - 5)),
		...
		reloadData: function(t) {
			function e() {
				...
				i.$_paymentType.html(t.paymentType)
			}
			$("#grid").clearGridData();
			var i = this;
			originalData = t;
		...
		enableEdit: function() {
			if (!disEditable) {
				...
				this.$_orderName.removeAttr("disabled").removeClass("ui-input-dis");
		...
		addEvent: function() {
			...
			$(".wrapper").on("click", "#save", function(e) {
				...
				if (i) {
					...
					Public.ajaxPost(sheet_add+"?act=add", {
			...
			$(".wrapper").on("click", "#savaAndAdd", function(e) {
				...
				i && Public.ajaxPost(sheet_add+"?act=addnew", {
					postData: JSON.stringify(i)
				}, function(e) {
					if (200 === e.status) {
